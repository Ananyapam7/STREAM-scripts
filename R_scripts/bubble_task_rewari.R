...
packagelist <- c('readr', 'purrr', 'stringr', 'dplyr', 'lubridate', 'jsonlite', 'rjson', 'xlsx')
...
topLevelFolder <- "/home/ananyapam/Projects/STREAM/data/rewari"
...
for (num_folder in 1:numberOfFolders) {
  tryCatch({
    ...
    child_ids[num_folder] <- as.numeric(thisFolder)
    xlFileNames <- list.files(path = thisFolder, pattern="bubble.*\\.csv$", full.names=TRUE)
    ...
    if (length(xlFileNames) > 0) {
      ...
      #first_sheet_data <- read_csv(xlFileNames[1], sheet = sheet_names[1], col_names = FALSE)
